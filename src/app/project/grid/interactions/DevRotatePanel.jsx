  ...
  return (
    <HudPanel title="Grid Rotations" positionClass="fixed top-28 right-4">
      ...
      <HudSection heading="X Axis (Left / Mid / Right)">
        <HudRow name="Back" align="right" disabled={disabled} onCw={() => api?.x_left(+1)}  onCcw={() => api?.x_left(-1)} />
        <HudRow name="Mid"  align="right" disabled={disabled} onCw={() => api?.x_mid(+1)}   onCcw={() => api?.x_mid(-1)} />
        <HudRow name="Front" align="right" disabled={disabled} onCw={() => api?.x_right(+1)} onCcw={() => api?.x_right(-1)} />
      </HudSection>

      {/* Y Axis */}
      <HudSection heading="Y Axis (Front / Mid / Back)">
        <HudRow name="Bottom" align="right" disabled={disabled} onCw={() => api?.y_front(+1)} onCcw={() => api?.y_front(-1)} />
        <HudRow name="Mid" align="right" disabled={disabled} onCw={() => api?.y_mid(+1)}   onCcw={() => api?.y_mid(-1)} />
        <HudRow name="Top" align="right" disabled={disabled} onCw={() => api?.y_back(+1)}  onCcw={() => api?.y_back(-1)} />
      </HudSection>

      {/* Z Axis */}
      <HudSection heading="Z Axis (Bottom / Mid / Top)" divider={false}>
        <HudRow name="Right" align="right" disabled={disabled} onCw={() => api?.z_bottom(+1)} onCcw={() => api?.z_bottom(-1)} />
        <HudRow name="Mid" align="right" disabled={disabled} onCw={() => api?.z_mid(+1)}    onCcw={() => api?.z_mid(-1)} />
        <HudRow name="Left" align="right" disabled={disabled} onCw={() => api?.z_top(+1)}    onCcw={() => api?.z_top(-1)} />
      </HudSection>
